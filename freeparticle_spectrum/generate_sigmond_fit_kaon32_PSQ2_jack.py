...
corr_paths = ["/latticeQCD/raid6/ruairi/freeparticle_energies/mom_averaging/avgbins/avgcorr_32_860_su_mesons_PSQ2"]
...
while tmax > 30:
    while tmin < (tmax - 6):
        dofit(tasks, operator, "kaon", str(tmin), str(tmax), tsse, "LMDer", inputdir + "fits/PSQ2/kaon_32_860_PSQ2_tsse_tmin" + str(tmin) + "tmax" + str(tmax) + "_jack.agr", psq, energies, "E1_kaon_4_35P0tsgs", "Jackknife")
        tmin+=1
    tmax-=1
    tmin=3
...
while tmax > 30:
    while tmin < (tmax - 15):
        dofit(tasks, operator, "kaon", str(tmin), str(tmax), tste, "LMDer", inputdir + "fits/PSQ2/kaon_32_860_PSQ2_tste_tmin" + str(tmin) + "tmax" + str(tmax) + "_jack.agr", psq, energies, "E1_kaon_4_35P0tsgs", "Jackknife")
        tmin+=1
    tmax-=1
    tmin=3
...
while tmax > 30:
    while tmin < (tmax - 15):
        dofit(tasks, operator, "kaon", str(tmin), str(tmax), tsgs, "LMDer", inputdir + "fits/PSQ2/kaon_32_860_PSQ2_tsgs_tmin" + str(tmin) + "tmax" + str(tmax) + "_jack.agr", psq, energies, "E1_kaon_4_35P0tsgs", "Jackknife")
        tmin+=1
    tmax-=1
    tmin=3
