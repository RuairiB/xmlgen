...
corr_paths = ["/latticeQCD/raid6/ruairi/freeparticle_energies/mom_averaging/avgbins/avgcorr_24_840_su_mesons_PSQ1"]
proj_name = "fit_24_840_kaon_su_meson_PSQ1"
inputdir = "/latticeQCD/raid6/ruairi/freeparticle_energies/SH_fits/24^3/kaon/"
logfile = inputdir + "log_fit_24_840_kaon_su_meson_PSQ1_boot.log"
...
while tmax > 30:
    while tmin < (tmax - 6):
        dofit(tasks, "GenIrreducible", operator, "kaon", str(tmin), str(tmax), tsseC, "Minuit2", inputdir + "fits/PSQ1/kaon_24_840_PSQ1_tsseC_tmin" + str(tmin) + "tmax" + str(tmax) + "_boot.agr", "1", energies, "E1_ref_5_35P0tsgs", "Bootstrap")
        tmin+=1
    tmax-=1
    tmin=3
...
while tmax > 30:
    while tmin < (tmax - 15):
        dofit(tasks, "GenIrreducible", operator, "kaon", str(tmin), str(tmax), tsteC, "Minuit2", inputdir + "fits/PSQ1/kaon_24_840_PSQ1_tsteC_tmin" + str(tmin) + "tmax" + str(tmax) + "_boot.agr", "1", energies, "E1_ref_5_35P0tsgs", "Bootstrap")
        tmin+=1
    tmax-=1
    tmin=3
...
while tmax > 30:
    while tmin < (tmax - 15):
        dofit(tasks, "GenIrreducible", operator, "kaon", str(tmin), str(tmax), tsgs, "Minuit2", inputdir + "fits/PSQ1/kaon_24_840_PSQ1_tsgs_tmin" + str(tmin) + "tmax" + str(tmax) + "_boot.agr", "1", energies, "E1_ref_5_35P0tsgs", "Bootstrap")
        tmin+=1
    tmax-=1
    tmin=3

    
writesamplings(tasks, energies, inputdir + "energies/kaon_24_840_PSQ1_boot", sampling="Bootstrap")
    
indent(root)
filename = str(inputdir + "input_fit_24_840_kaon_su_meson_PSQ1_boot.xml")
tree.write(filename)
