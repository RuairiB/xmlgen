...
corr_paths = ["/latticeQCD/raid6/ruairi/freeparticle_energies/mom_averaging/avgbins/avgcorr_24_840_du_mesons_PSQ2"]
proj_name = "fit_24_840_pion_du_meson_PSQ2"
inputdir = "/latticeQCD/raid6/ruairi/freeparticle_energies/SH_fits/24^3/pion/"
logfile = inputdir + "log_fit_24_840_pion_du_meson_PSQ2_boot.log"
...
while tmax > 30:
    while tmin < (tmax - 6):
        dofit(tasks, "GenIrreducible", operator, "pion", str(tmin), str(tmax), tsseC, "Minuit2", inputdir + "fits/PSQ2/pion_24_840_PSQ2_tsseC_tmin" + str(tmin) + "tmax" + str(tmax) + "_boot.agr", psq, energies, "E1_ref_5_35P0tsgs", "Bootstrap")
        tmin+=1
    tmax-=1
    tmin=3
...
while tmax > 30:
    while tmin < (tmax - 10):
        dofit(tasks, "GenIrreducible", operator, "pion", str(tmin), str(tmax), tsteC, "Minuit2", inputdir + "fits/PSQ2/pion_24_840_PSQ2_tsteC_tmin" + str(tmin) + "tmax" + str(tmax) + "_boot.agr", psq, energies, "E1_ref_5_35P0tsgs", "Bootstrap")
        tmin+=1
    tmax-=1
    tmin=3
...
while tmax > 30:
    while tmin < (tmax - 10):
        dofit(tasks, "GenIrreducible", operator, "pion", str(tmin), str(tmax), tsgs, "Minuit2", inputdir + "fits/PSQ2/pion_24_840_PSQ2_tsgs_tmin" + str(tmin) + "tmax" + str(tmax) + "_boot.agr", psq, energies, "E1_ref_5_35P0tsgs", "Bootstrap")
        tmin+=1
    tmax-=1
    tmin=3

    
writesamplings(tasks, energies, inputdir + "energies/pion_24_840_PSQ2_boot", sampling="Bootstrap")
    
indent(root)
filename = str(inputdir + "input_fit_24_840_pion_du_meson_PSQ2_boot.xml")
tree.write(filename)
