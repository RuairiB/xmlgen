...
while tmax > 30:
    while tmin < (tmax - 6):
        dofit(tasks, "BasicLaph", "kaon P=(0,0,0) A1u_1 SS_0", "kaon", str(tmin), str(tmax), tsseC, "Minuit2", inputdir + "fits/PSQ0/kaon_24_840_PSQ0_tsseC_tmin" + str(tmin) + "tmax" + str(tmax) + "_jack.agr", "0", energies, "E1_ref_5_35P0tsgs", "Jackknife")
        tmin+=1
    tmax-=1
    tmin=3
...
while tmax > 30:
    while tmin < (tmax - 15):
        dofit(tasks, "BasicLaph", "kaon P=(0,0,0) A1u_1 SS_0", "kaon", str(tmin), str(tmax), tsteC, "Minuit2", inputdir + "fits/PSQ0/kaon_24_840_PSQ0_tsteC_tmin" + str(tmin) + "tmax" + str(tmax) + "_jack.agr", "0", energies, "E1_ref_5_35P0tsgs", "Jackknife")
        tmin+=1
    tmax-=1
    tmin=3
...
while tmax > 30:
    while tmin < (tmax - 15):
        dofit(tasks, "BasicLaph", "kaon P=(0,0,0) A1u_1 SS_0", "kaon", str(tmin), str(tmax), tsgs, "Minuit2", inputdir + "fits/PSQ0/kaon_24_840_PSQ0_tsgs_tmin" + str(tmin) + "tmax" + str(tmax) + "_jack.agr", "0", energies, "E1_ref_5_35P0tsgs", "Jackknife")
        tmin+=1
    tmax-=1
    tmin=3

    
writesamplings(tasks, energies, inputdir + "energies/kaon_24_840_PSQ0_jack", sampling="Jackknife")
    
indent(root)
filename = str(inputdir + "input_fit_24_840_kaon_su_meson_PSQ0_jack.xml")
tree.write(filename)
