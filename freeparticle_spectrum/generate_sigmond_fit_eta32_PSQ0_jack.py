...
logfile = inputdir + "log_fit_32_860_eta_uu_meson_PSQ0_jack.log"
...
while tmax > 20:
    while tmin < (tmax - 6):
        dofit(tasks, "BasicLaph", "eta P=(0,0,0) A1up_1 SS_0", "eta", str(tmin), str(tmax), tsse, "Minuit2", inputdir + "fits/PSQ0/eta_32_860_PSQ0_tsse_tmin" + str(tmin) + "tmax" + str(tmax) + "_jack.agr", "0", energies, "E1_tmin8tmax34P0tsgs", "Jackknife")
        tmin+=1
    tmax-=1
    tmin=3
...
while tmax > 20:
    while tmin < (tmax - 15):
        dofit(tasks, "BasicLaph", "eta P=(0,0,0) A1up_1 SS_0", "eta", str(tmin), str(tmax), tste, "Minuit2", inputdir + "fits/PSQ0/eta_32_860_PSQ0_tste_tmin" + str(tmin) + "tmax" + str(tmax) + "_jack.agr", "0", energies, "E1_tmin8tmax34P0tsgs", "Jackknife")
        tmin+=1
    tmax-=1
    tmin=3
...
while tmax > 20:
    while tmin < (tmax - 15):
        dofit(tasks, "BasicLaph", "eta P=(0,0,0) A1up_1 SS_0", "eta", str(tmin), str(tmax), tsgs, "Minuit2", inputdir + "fits/PSQ0/eta_32_860_PSQ0_tsgs_tmin" + str(tmin) + "tmax" + str(tmax) + "_jack.agr", "0", energies, "E1_tmin8tmax34P0tsgs", "Jackknife")
        tmin+=1
    tmax-=1
    tmin=3

    
writesamplings(tasks, energies, inputdir + "energies/eta_32_860_PSQ0_jack", sampling="Jackknife")
    
indent(root)
filename = str(inputdir + "input_fit_32_860_eta_uu_meson_PSQ0_jack.xml")
tree.write(filename)
