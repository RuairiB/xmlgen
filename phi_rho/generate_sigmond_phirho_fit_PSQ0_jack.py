...
corr_paths = ["/home/ruairi/research/phi_rho/working/rotated_matrices/rotcorr_single_phi_rho_P0_0_0_1_jack"]

proj_name = "phi_rho_fit_PSQ0_jack"
inputdir = "/home/ruairi/research/phi_rho/working/"
logfile = inputdir + "log_fit_PSQ0_jack.log"
...
while level < 7:
    for fitfn in fit_types:
        ...
        while tmax > 5:
            while tmin < (tmax - 2):
                dofit(tasks, "GenIrreducible", operator0 + str(level), "phirho" + str(level), str(tmin), str(tmax), fitfn, "LMDer", inputdir + "fits/PSQ0/phi_rho_PSQ" + psq + "_" + shortform(fitfn) + "_tmin" + str(tmin) + "tmax" + str(tmax) + "level" + str(level) + "_jack.agr", psq, energies, "none", "Jackknife")
                tmin += 1
            tmax -= 1
            tmin = 0
...
writesamplings(tasks, energies, inputdir + "energies/phi_rho_PSQ0_jack", sampling="Jackknife")
    
indent(root)
filename = str(inputdir + "input_phi_rho_fit_PSQ0_jack.xml")
tree.write(filename)
