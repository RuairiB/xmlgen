...
logfile = inputdir + "log_fit_PSQ1_boot.log"
...
while level < 6:
    for fitfn in fit_types:
        ...
        while tmax > 5:
            while tmin < (tmax - 2):
                dofit(tasks, "GenIrreducible", operator0 + str(level), "phirho" + str(level), str(tmin), str(tmax), fitfn, "LMDer", inputdir + "fits/PSQ1/phi_rho_PSQ" + psq + "_" + shortform(fitfn) + "_tmin" + str(tmin) + "tmax" + str(tmax) + "level" + str(level) + "_boot.agr", psq, energies, "none", "Bootstrap")
                tmin += 1
            tmax -= 1
            tmin = 0
...
writesamplings(tasks, energies, inputdir + "energies/phi_rho_PSQ1_boot", sampling="Bootstrap")
    
indent(root)
filename = str(inputdir + "input_phi_rho_fit_PSQ1_boot.xml")
tree.write(filename)
